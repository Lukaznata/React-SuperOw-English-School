...
export default function ButtonPagarValor() {
      ...
      if (err.response?.data?.detail) {
        if (Array.isArray(err.response.data.detail)) {
          const errorMessages = err.response.data.detail
            .map((e: any) => `Campo: ${e.loc?.join(".")} - ${e.msg}`)
            .join(", ");
  ...
  return (
    <>
      <button
        ...
        className={`
        w-full p-3 font-bold cursor-pointer text-red-800 
        bg-white rounded-lg shadow-lg
        flex items-center justify-center 
        hover:bg-red-900 hover:text-white transition-all duration-300
        `}
      >
        Adicionar novo saldo a pagar
      </button>
      ...
      {showToast && (
        ...
        >
          ✓ Saldo a pagar cadastrado com sucesso!
        </div>
      )}
      ...
      {mostrarModal && (
        ...
        >
          <div className="bg-white p-6 rounded-lg w-96 shadow-xl">
            <h2 className="text-xl font-bold mb-4 text-[#661212]">
              Novo Saldo a Pagar
            </h2>
